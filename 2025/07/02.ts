...
function findAllIndices(arr: number[], notCondition: number) {
  const indices = [];
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i] !== notCondition) {
      indices.push(i);
    }
  }
  return indices;
...
(async () => {
  ...
  const dp: number[][] = Array.from({ length: inputs.length }, () => {
  ...
  for (let y = 0; y < inputs.length - 1; y += 1) {
    const beamIndexes = findAllIndices(dp[y], 0);
    // check next row
    for (let i = 0; i < beamIndexes.length; i += 1) {
      const char = inputs[y + 1][beamIndexes[i]];
      if (char === ".") {
        dp[y + 1][beamIndexes[i]] += dp[y][beamIndexes[i]];
      }
      if (char === "^") {
        if (beamIndexes[i] - 1 >= 0) {
          dp[y + 1][beamIndexes[i] - 1] += dp[y][beamIndexes[i]];
        }
  ...
  const finalRow = dp.pop() as number[];
  const total = MathUtil.add(...finalRow);
