...
let calculatedSnailfish = inputs[0];
for (let i = 1; i < inputs.length; i += 1) {
  const currentSnailfish = inputs[i];
  const snailfish = addition(calculatedSnailfish, currentSnailfish);
  const newSnailfish = action(snailfish);
  calculatedSnailfish = newSnailfish;
}

const total = calculate(calculatedSnailfish);
