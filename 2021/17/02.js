...
function calculatePossibilities(xVelocities, minY) {
  ...
  for (let x = xMin; x <= xMax; x += 1) {
    for (let y = minY; y < 1000; y += 1) {
      ...
      if (simulation.answer) {
        possibilities.add(simulation.coords);
      }
    }
  }
...
const minY = inputs.y.min;
const xRange = calculateXVelocities();
const possibilities = [...calculatePossibilities(xRange, minY)];
